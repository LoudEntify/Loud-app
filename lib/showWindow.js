...
// ── THE KIT CHECK HANDOVER (Finding 1) ────────────────────────
// How long before showtime the countdown overlay appears.
export const HANDOVER_COUNTDOWN_SECONDS = 60;

/**
 * What Kit Check should be doing right now.
 *
 * Pulled out of components/KitCheck.jsx as a pure function so it can be
 * tested without a browser, a session or a database — which is exactly
 * what was missing when the countdown broke twice in a row:
 *
 *   round 1: it counted down from the WINDOW OPENING (T−30) instead of
 *            from showtime, throwing the artist on stage minutes early.
 *   round 2: it stopped firing entirely — `nextUpcomingShow` threw
 *            (the re-export/scope bug), and because Kit Check calls it
 *            inside an ASYNC EFFECT rather than during render, the throw
 *            became an unhandled rejection instead of a crash. `upcoming`
 *            stayed null and the whole condition was silently false.
 *
 * Both were one predicate nobody could exercise in isolation. Now it is
 * one function with a name, and scripts/window-tests.mjs runs it.
 *
 * Returns:
 *   'none'      no show, or its broadcast window is not open
 *   'waiting'   window open, more than the countdown away
 *   'countdown' inside the final minute — show the overlay
 *   'due'       showtime reached — hand over to the stage
 */
export function handoverState(show, now = Date.now(), countdownSeconds = HANDOVER_COUNTDOWN_SECONDS) {
  const base = { status: 'none', secondsToShowtime: null, countdown: null };
  if (!show?.id || !show?.slated_at) return base;
  if (!isWindowOpen(show, now)) return base;

  const slated = new Date(show.slated_at).getTime();
  if (Number.isNaN(slated)) return base;

  // Ceil, so 59.4s reads as 60 rather than 59 and the overlay does not
  // appear to skip a number on its first tick.
  const secondsToShowtime = Math.ceil((slated - now) / 1000);

  if (secondsToShowtime > countdownSeconds) {
    return { status: 'waiting', secondsToShowtime, countdown: null };
  }
  if (secondsToShowtime > 0) {
    return { status: 'countdown', secondsToShowtime, countdown: secondsToShowtime };
  }
  // Clamped at zero: opening Kit Check AFTER showtime (window still
  // open) must read 0 and hand over immediately, never a negative number.
  return { status: 'due', secondsToShowtime, countdown: 0 };
}

/**
 * Can the artist take themselves to the stage by hand right now?
 *
 * The BROADCAST window (T−30 → close), deliberately — not the show
 * window. Stated because it is a choice: the manual button is "take me
 * to my show now", and the same T−30 that lets an artist set up and
 * soundcheck is when going to the stage becomes a reasonable thing to
 * want. It matches canGoLive's own lower bound (lib/showState.js), so
 * the console's GO LIVE and Kit Check's cannot disagree about when
 * either is available.
 */
export function canHandOverNow(show, now = Date.now()) {
  return isWindowOpen(show, now);
}
...
export function humanCountdown(ms) {
  if (ms === null) return '';
  if (ms <= 0) return 'now';
  // Seconds under a minute. Without this the last 59 seconds before a
  // window opens all read "in 0m" -- on the Go Live button, which is
  // exactly the minute an artist is standing there watching it, and on
  // the viewer's countdown to showtime.
...
// Structured parts rather than a formatted string, so the homepage can
// render days/hours/minutes/seconds as separate elements and the caller
// decides the typography.
//
// `scale` is the largest unit that currently matters, which is what
// makes the countdown change character as it approaches: days a week
// out, then hours, then minutes, then a second-by-second clock in the
// last minute. A single MM:SS format cannot do that -- it reads
// "4320:00" three days out, which is not a countdown, it is a number.
//
// Pure and in this module for the same reason humanCountdown is: it has
// to be runnable without a browser or a Supabase client.
export function countdownParts(ms) {
  const done = !Number.isFinite(ms) || ms <= 0;
  const total = done ? 0 : Math.floor(ms / 1000);
  const days = Math.floor(total / 86400);
  const hours = Math.floor((total % 86400) / 3600);
  const minutes = Math.floor((total % 3600) / 60);
  const seconds = total % 60;
  let scale = 'seconds';
  if (days > 0) scale = 'days';
  else if (hours > 0) scale = 'hours';
  else if (minutes > 0) scale = 'minutes';
  return { total, days, hours, minutes, seconds, scale, done };
}
