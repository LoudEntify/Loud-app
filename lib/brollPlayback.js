...
export function createBrollPublisher({ room, onEnded, onError }) {
  ...
  return {
    ...
    async play({ clip, accessToken }) {
      ...
      // ── THE CUT WAITS FOR FRAMES, NOT FOR A PROMISE ───────────
      // replaceTrack() resolving means the sender accepted the track. It
      // says nothing about whether anything has been encoded. The caller
      // fires the cut on this function returning, so returning early is
      // the same as cutting to a dead layer.
      const confirmed = await waitForFrames(publication);
      logHealthEvent('broll_frames_confirmed', {
        clipId: clip.id,
        ok: confirmed.ok,
        waitedMs: confirmed.waitedMs,
        reason: confirmed.reason ?? null,
      });
      if (!confirmed.ok) {
        // No frames means there is nothing to cut to. Coming back down
        // is better than putting a dead sender on air and leaving the
        // audience to look at it.
        await this.stop();
      ...
      logHealthEvent('broll_published', {
        ...
        via: 'swap',
        // How long the sender took to produce its first frame after the
        // swap. This is the number the blank stage is measured by.
