...
//
// NOTE: this assumes the frame it's given genuinely has its claimed
// dimensions (no anamorphic distortion baked in). A capture-card source
// whose raw getSettings() reading fails useNativeIsLandscape's
// plausibility check (lib/useSourceDimensions.js) is NOT fixed here --
// resampling a squeezed buffer downstream was tried and overcorrected on
// real hardware (Sony via capture card: faces went from narrow to
// stretched wide, the resample factor didn't match the driver's actual
// distortion). The real fix is upstream, at acquisition: re-request the
// device's native landscape capability instead of a portrait ideal (see
// landscapeNativeCaptureOptions, lib/useSourceDimensions.js) so the
// buffer that reaches this processor was never squeezed in the first
// place -- this crop-only logic then just works, same as any other
// genuinely-landscape source.
...
export function createPortraitProcessor(degrees = 0) {
  let videoEl = null;
  let canvas = null;
  let ctx = null;
  let rvfcHandle = null;
  ...
  function drawFrame() {
    ...
    if (vw && vh) {
      try {
        // Rotated-space dimensions -- what the frame's own width/height
        // become after the chosen rotation, before any crop.
        const rw = swapped ? vh : vw;
        const rh = swapped ? vw : vh;
        const { outW, outH } = fitPortraitTarget(rw, rh);

        // Map the rotated-space crop back to a source rectangle in RAW
        // (pre-rotation) pixels -- a swapped rotation means the rotated
        // width axis came from the raw HEIGHT axis and vice versa, so
        // the crop rectangle's width/height swap back too.
        const sw = swapped ? outH : outW;
        const sh = swapped ? outW : outH;
        const sx = (vw - sw) / 2;
        const sy = (vh - sh) / 2;
