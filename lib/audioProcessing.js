...
export async function createPilotAudioTrack(options = {}) {
  ...
  // Mic mute (fix 2a) -- sits BETWEEN vocalBus and outputBus, which is
  // the only position that satisfies all three requirements at once:
  //
  //   * backing track keeps playing to viewers -- it joins at outputBus
  //     (loadBackingTrack -> delayNode -> outputBus below), DOWNSTREAM of
  //     this node, so muting the mic cannot touch it. The old behavior
  //     muted the published MediaStreamTrack itself, i.e. the whole mix,
  //     which silenced the backing track too.
  //   * the input meter keeps moving while muted -- inputAnalyser taps
  //     inputGain, UPSTREAM of here, so the artist can still see their
  //     own voice registering and knows the mic is live but not going
  //     out.
  //   * vocal monitoring is unaffected -- monitorGain taps vocalBus, not
  //     this node. Zeroing vocalBus itself would have been one node
  //     fewer but would also have killed the artist's own headphone
  //     monitor as a side effect of muting.
  const micMuteGain = audioContext.createGain();
  micMuteGain.gain.value = 1;

  vocalBus.connect(micMuteGain);
  micMuteGain.connect(outputBus);
  ...
  return {
    ...
    nodes: {
      ...
      vocalBus, micMuteGain, outputBus, outputAnalyser, monitorGain,
    },
  };
}
...
// Mic mute (fix 2b) -- the ONLY correct way to mute the performer's
// voice. Muting the published MediaStreamTrack (`track.enabled = false`,
// what this used to do) silences the entire Web Audio mix, backing
// track included, because the published track IS the mix's output.
//
// Ramped rather than a `.value =` step, same 15ms time constant and same
// reasoning as tuneEffectsBypass above: an instant gain change is a
// waveform discontinuity, which is audible as a click.
export function tuneMicMuted(nodes, muted) {
  const t = nodes.micMuteGain.context.currentTime;
  nodes.micMuteGain.gain.setTargetAtTime(muted ? 0 : 1, t, 0.015);
}
...
// --- Case 3: backing track ---------------------------------------------
// Two sources, one path. The artist either picks a file from their own
// device -- no upload, no server, no database, the bytes never leave
// this browser -- or loads one from their uploaded library
// (docs/mvp2_01_backing_tracks.sql), which arrives here as a Blob
// fetched from private storage. loadBackingTrack takes either, because
// a File IS a Blob and this function only ever wanted bytes.
//
// Everything below is identical for both, which is deliberate: the two
// sources produce the same sha256 and therefore the same cue sheet, and
// a decode path that treated them differently would be the place that
// stopped being true. It's decoded locally and mixed directly into outputBus,
...
export async function loadBackingTrack(audioContext, outputBus, file) {
  ...
  // ── TASK 3 — THE DECK SAYS WHEN IT CHANGED ────────────────────
  // `playing` and `pausedAt` are private to this closure, and until now
  // the only way to learn they had moved was to ask getElapsed()/
  // isPlaying() on a timer. That is why show_session_state recorded a
  // pause up to five seconds after it happened: nothing pushed, so the
  // poller was the whole story.
  //
  // This is the push. One optional listener, called AFTER the state it
  // describes has settled, so a listener that immediately reads back
  // isPlaying()/getElapsed() sees the new values rather than the old
  // ones. It is deliberately a bare callback and not an event system:
  // there is exactly one legitimate subscriber (lib/audioHost.js, which
  // fans out to anyone who cares), and giving the audio engine a
  // subscriber list would invite work to be hung off the playback path.
  //
  // Nothing here awaits the listener or reads its return value. A
  // subscriber that throws must not be able to stop the music, which is
  // why the try/catch is inside notify and not around the call sites.
  let transitionListener = null;
  function notifyTransition(kind) {
    if (!transitionListener) return;
    try { transitionListener(kind); } catch { /* a listener must never break playback */ }
  }
  ...
  function play() {
    ...
    sourceNode.onended = () => {
      if (playing && audioContext.currentTime - startedAt >= audioBuffer.duration - 0.05) {
        ...
        // The one transition with no caller behind it. A track that runs
        // out is as real a stop as a tapped one, and without this it
        // would be the only kind still discovered late by the poller.
        notifyTransition('ended');
      }
    };
  ...
  function seek(timeSeconds) {
    ...
    playing = false;
    if (wasPlaying) play();
    // Fires after the inner play() has already fired its own. Two
    // notifications, one write: the subscriber writes on the first and
    // finds nothing changed on the second (components/AudioHostProvider.jsx
    // suppresses a write when neither the state nor the playhead moved).
    notifyTransition('seek');
  }
  ...
  return {
    ...
    getElapsed,
    isPlaying: () => playing,
    // Set by lib/audioHost.js when this player is adopted, cleared when
    // it is replaced or released. Null is the normal state for a player
    // nobody has adopted yet — a deck that is decoded but not yet handed
    // over has no session to report to.
    setTransitionListener: (fn) => { transitionListener = typeof fn === 'function' ? fn : null; },
    disconnect: () => {
      stop();
