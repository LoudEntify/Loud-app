...
import { isShowWindowOpen, showWindowClosesAt } from './showWindow';
...
export async function fetchLiveShows() {
  try {
    ...
    const now = Date.now();
    return (data || [])
      // Product Ruling 1 — a show is in Live Now only INSIDE its own
      // window. This used to be `slated_at <= now` with no upper bound
      // at all, so a show nobody ended stayed in Live Now indefinitely,
      // advertising a room with nobody in it. isShowWindowOpen is the
      // same function the artist's own GO LIVE button and the join-show
      // route use.
      .filter((s) => isShowWindowOpen(s, now))
...
export async function fetchUpcomingShows({ limit = 20 } = {}) {
  try {
    ...
    return (data || [])
      ...
      // Product Ruling 1 — and its window must not already have closed.
      // Belt and braces with the line above (a future start implies an
      // open window), kept explicit so the rule reads the same here as
      // everywhere else rather than being implied.
      .filter((s) => showWindowClosesAt(s) > now)
