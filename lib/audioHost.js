...
// ── TASK 3 — PLAYBACK TRANSITIONS, ON THEIR OWN CHANNEL ───────
// play / pause / stop / seek / natural end, pushed by the player itself
// (lib/audioProcessing.js) the instant they happen.
//
// ── WHY THIS IS NOT JUST emit() ───────────────────────────────
// Because emit() bumps host.version, and host.version is what React
// components subscribe to through useSyncExternalStore. Routing playback
// transitions through it would re-render every subscriber on every tap of
// play, pause and every drag of the waveform — turning a database-latency
// fix into a rendering change.
//
// That matters more than usual right now: the round-2 CPU investigation is
// PAUSED and UNATTRIBUTED, with the rAF/waveform loop and the decoded
// buffer as the two surviving candidates. Adding re-renders to the deck
// while that is open would contaminate the very measurements meant to
// settle it. A separate listener set costs one Set and keeps this round's
// change invisible to anything that renders.
//
// WHAT it changed is deliberately not carried in the payload beyond a
// label. Subscribers read the player for the actual values — one clock,
// as playerPositionMs() has always insisted — so there is no second copy
// of the playhead to disagree with the first.
const transitionListeners = new Set();

/**
 * Subscribe to playback transitions. Returns an unsubscribe function.
 * The callback receives the kind: 'play' | 'pause' | 'stop' | 'seek' | 'ended'.
 */
export function subscribeAudioTransitions(fn) {
  transitionListeners.add(fn);
  return () => transitionListeners.delete(fn);
}

function emitTransition(kind) {
  transitionListeners.forEach((fn) => {
    try { fn(kind); } catch { /* a bad subscriber must not break audio */ }
  });
}
...
export function setBackingPlayer(player, trackHash, trackName) {
  if (host.player && host.player !== player) {
    // Detached BEFORE the stop, not after: stopping the outgoing player
    // fires its own 'stop' transition, and a listener still attached to a
    // player that is being thrown away would report that stop as if it
    // were the deck's current state — a 'stopped' write landing after the
    // 'loaded' one, describing a player nobody is holding any more.
    try { host.player.setTransitionListener?.(null); } catch { /* already gone */ }
    try { host.player.stop(); host.player.disconnect?.(); } catch { /* already gone */ }
  }
  host.player = player || null;
  // The single place a player is wired to the transition channel. Doing it
  // here rather than at the decode sites means a player is connected
  // exactly when it becomes the host's, which is also exactly when there
  // is a session for its transitions to be written to.
  try { host.player?.setTransitionListener?.(emitTransition); } catch { /* not a player we recognise */ }
...
export function releaseAudioHost(reason = 'unspecified') {
  // Same ordering as setBackingPlayer's replace path, for the same
  // reason: the stop below is teardown, not a performance decision, and
  // it must not be reported as one.
  try { host.player?.setTransitionListener?.(null); } catch { /* fine */ }
