...
// ─────────────────────────────────────────────────────────────
// TASK 3 — which way the camera is facing, remembered.
//
// PRD: Multi-camera / Kit Check    S&I: Real-time media
//
// Stored separately from the credential because it is a different KIND
// of fact with a different lifetime: the credential identifies the
// device, this is a per-device preference that must outlive a reconnect.
//
// ── WHAT THIS DELIBERATELY DOES NOT TOUCH ─────────────────────
// The SLOT ROLE. wide/close/side is an INTENT — what this camera is for
// in the shot grammar — decided by the artist when they invite the
// camera, stored on the pairing row, and handed back by
// /api/camfeed/session on every poll. Which lens is pointing at the
// world is a property of the DEVICE and has nothing to do with it.
//
// Keeping them apart is the requirement: a phone that turns around is
// still the SIDE camera. If facing ever started influencing role, the
// slot grammar would have become device-ID-based by the back door.
const FACING_KEY = 'loudentify.camfeed.facing';

export function readFacingMode() {
  try {
    const v = window.localStorage.getItem(FACING_KEY);
...
export function saveFacingMode(mode) {
  if (mode !== 'user' && mode !== 'environment') return;
  try { window.localStorage.setItem(FACING_KEY, mode); } catch { /* private browsing */ }
}
