...
//                        exposes, extended with meta.sourceRole: an
//                        EXPLICIT role override that bypasses
//                        resolveSourceRole entirely. Cues carry an
//                        already-resolved role intent (slot_role) --
//                        meta.framingHint is a different vocabulary
//                        (a SHOT_TYPES key, so a technique like pan can
//                        inherit its framing step's source array) and
//                        must never be fed a role value; SHOT_TYPES has
//                        no entries named 'main'/'wide'/'close'/'side',
//                        so resolveSourceRole would silently return null
//                        and resolveTargetIdentity's null-role branch
//                        falls through to the main performer -- a real
//                        bug found via a device-test health-events
//                        capture (a 'side' cue resolving to
//                        contestant-a-*, no cue_fallback logged since
//                        the live-role gate below had already passed).
//                        meta.params carries cue.motion mapped onto the
//                        direction/vertigo params ShotRenderer already
//                        reads. Must return the built command (for its
//                        resolved targetIdentity) so cueDirector can log
//                        what actually fired.
//   getAvailableRoles, — () => roles currently live for this slot
//   getPlayerState,    — () => { elapsedMs, isPlaying } — polled, never cached
//   suspendAuto,       — () => void — auto.suspendFor('cue'). A NAMED
//                        owner: a b-roll clip cued inside cue mode takes
//                        its own hold, and releasing that one must not
//                        release this one (lib/autoDirector.js).
//   resumeAuto,        — () => void — auto.resumeFor('cue')
// })
export function createCueDirector({ sheet, fireShot, getAvailableRoles, getPlayerState, suspendAuto, resumeAuto }) {
  ...
  function fireCue(cue) {
    ...
    // A b-roll cue that NAMES a clip does not need 'broll' to already be
    // live -- it is what makes it live. Every other cue still requires
    // its role to be publishing, which is what sends a cue naming a dead
    // camera down the fallback path instead of cutting to nothing.
    const startsItsOwnClip = cue.slot_role === 'broll' && !!cue.clip_id;
    if (roles.includes(cue.slot_role) || startsItsOwnClip) {
      const params = cue.motion
        ...
        : undefined;
      const command = fireShot(cue.shot_type, 'cue', {
        sourceRole: cue.slot_role,
        params,
        // Only ever set on a b-roll cue (validateCue enforces that). The
        // consumer starts this clip rather than building a plain command.
        clipId: cue.clip_id,
      });
      logHealthEvent('cue_fired', {
        ...
        clipId: cue.clip_id ?? null,
        // null for a b-roll cue that started a clip: the cut is fired by
        // the playback path once the track is actually published, not
        // synchronously here. See LiveDemo's fireCueShot.
    ...
    if (fallbackShot) {
      // No sourceRole/framingHint override here -- cue.slot_role is
      // confirmed not live at this point (that's why we're in the
      // fallback branch at all), so this deliberately falls through to
      // resolveSourceRole's own normal resolution against fallbackShot's
      // (e.g. 'wide') declared source array, same as firing that shot
      // plainly anywhere else.
      const command = fireShot(fallbackShot, 'cue', {});
