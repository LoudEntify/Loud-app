    ...
    async function sample() {
      ...
      try {
        ...
        logHealthEvent('pub_stats', {
          // ⚠️ CARRIED IN THE PAYLOAD AS WELL AS THE CONTEXT.
          // health_events.participant_identity comes from the logging
          // CONTEXT, which is set once at mount and was empty before the
          // server had assigned an identity -- so a whole rehearsal's
          // encoder stats arrived unattributable. The context bug is
          // fixed, but a sample that cannot be attributed is worthless
          // and there is exactly one pilot, so it now also travels with
