...
export function planStaleShots({
  ...
  Object.entries(activeShot || {}).forEach(([slot, cmd]) => {
    ...
    if (
      ...
      // framingSuspendedAt is deliberately KEPT through the downgrade,
      // where framingSuspended is not. The flag is a state -- it is over,
      // the slot is downgraded now -- but the timestamp is the answer to
      // "how long has this camera been gone", which does not stop being a
      // question just because we stopped waiting for it.
      //
      // Stripping it made `awayMs` null on every resume that followed a
      // downgrade, so the one event that describes the longest outages
      // was the one that could not say how long they were. It is safe to
      // keep: the downgrade branch below requires `suspended`, which is
      // now false, so this cannot re-trigger.
      const { framingSuspended, ...rest } = cmd;
