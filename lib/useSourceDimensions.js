...
// card). The fix for that lives at acquisition, not in the crop
// processor: CamPage.jsx/LiveDemo.jsx re-request the device with
// landscapeNativeCaptureOptions (below) when this fires, which is why
// this hook rebinds and re-detects on TrackEvent.Restarted -- the same
// publication/trackSid keeps flowing through a restartTrack() call, so
// without this the hook would keep reporting its stale first read
// forever instead of picking up the (hopefully now-genuine) landscape
// dimensions on the re-acquired track. A plain width > height reading is
// unambiguous landscape with no squeeze implied (implausibleAspect stays
// false), same as a clean recognized portrait/landscape ratio.
export function useNativeIsLandscape(publication) {
  ...
  useEffect(() => {
    ...
    function runDetection() {
      detachRead?.();
      setState({ isLandscape: null, implausibleAspect: false });
      detachRead = readRawDimensionsOnce(track, (dims) => {
        if (!dims) {
...
// Constraint object to re-acquire a device with once its raw dims fail
// the plausibility check above -- asks for its actual native landscape
// capability instead of the portrait ideal every source is normally
// requested with, so the driver has no shape mismatch to paper over by
// squeezing in the first place. getCapabilities() isn't supported in
// every browser (notably older Safari) -- falls back to 1920x1080, the
// native mode confirmed on real hardware (Sony via capture card) for
// this class of device. No aspectRatio/portrait hint anywhere in this
// object, deliberately -- that's the whole point versus
// HIGH_RES_VIDEO_CAPTURE (CamPage.jsx/LiveDemo.jsx), which every OTHER
// source keeps using unchanged.
export function landscapeNativeCaptureOptions(mediaStreamTrack) {
