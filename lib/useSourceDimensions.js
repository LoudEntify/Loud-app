...
function watchTrackDimensions(track, onUpdate) {
  ...
  function handleLiveUpdate() {
    ...
    if (settings?.width && settings?.height) {
      onUpdate({ status: 'ready', width: settings.width, height: settings.height });
    }
  }
  ...
  function attemptInitialRead(attempt) {
    const settings = readSettings();
    if (settings?.width && settings?.height) {
      onUpdate({ status: 'ready', width: settings.width, height: settings.height, attempts: attempt });
      return;
    }
...
//
// Simpler contract than useSourceDimensions on purpose -- crop
// selection just needs a boolean with a safe default, not the
// detecting/failed distinction: returns null while detecting OR if
// detection ultimately fails (ShotTransformFrame's own `?? false`
// already treats null as "assume landscape", a reasonable fallback
// either way), or { width, height, isPortraitSource } once ready.
export function useTrackAspect(trackRef) {
  ...
  return { width: state.width, height: state.height, isPortraitSource: state.height > state.width };
}
