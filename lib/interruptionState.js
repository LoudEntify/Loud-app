...
// ── THE MERGED STATE, REPORTED ON RETURN ──────────────────────
// Rules 2 and 3 became one state (docs/INTERRUPTION_FEASIBILITY.md §7):
// camera paused, audio continuing, artist away from the screen. Minimise
// and lock are indistinguishable on the measured device, so the app does
// not claim to know which happened.
//
// ── WHY THIS IS A RETURN NOTICE AND NOT A LIVE ONE ────────────
// The state only exists while the artist is away from the screen, so
// there is no moment during it when they could read anything. That
// constraint is doing the hardest work in this file: it makes the
// message a REPORT OF WHAT JUST HAPPENED rather than a description of
// how the platform behaves. The distinction is the whole copy rule —
// "your microphone stayed on" is an observation of this device, this
// time; "minimising keeps you live" would be a promise about a platform
// measured once, on one handset, in one browser.
//
// ── WHY THE SECOND LINE EXISTS ────────────────────────────────
// A locked phone keeps broadcasting the artist's audio. That was ruled
// deliberately — the app cannot tell a phone set down from a phone in a
// pocket mid-song, and cutting a performer's voice because their screen
// went dark is the worse failure. Having ruled it, the artist has to be
// told plainly, because it is the one consequence they would never
// guess and the one they might have wanted to prevent.
//
// ── WHY THE DURATION EARNED ITS PLACE ─────────────────────────
// It answers the only question worth asking on return: how bad was
// that. Four seconds and two minutes are different situations, and the
// difference decides whether the artist carries on or says something to
// the room. Purely factual, about this device, this time.
//
// ── WHAT IS DELIBERATELY ABSENT ───────────────────────────────
// Anything about what the AUDIENCE saw. This measured the phone, not
// delivery. The app knows its own holding behaviour but not what
// arrived, and a line claiming the room saw a held frame would be a
// claim nobody can stand behind.

/** Below this, a glance at a notification would produce a card. */
export const AWAY_NOTICE_MIN_MS = 5000;
...
/**
 * The three lines shown on return from the merged state. Ordered by what
 * the artist needs first: what happened, the consequence they could not
 * have known, then how long.
 */
export function describeAwayReturn(awayForMs) {
  return {
    camera: 'Your camera paused while you were away.',
    microphone: 'Your microphone stayed on.',
    // Sampled at 1Hz, so this is accurate to about a second — which is
    // the granularity the sentence is read at anyway.
    duration: `You were away for ${formatAwayDuration(awayForMs)}.`,
  };
}
...
export function useCapabilityWatch({ audioContext, getTracks, enabled = true }) {
  ...
  // The merged state's episode: when it started, and what it came to.
  // Held in a ref while it is happening and published as state only when
  // it ENDS, because nothing can be shown to somebody who is not looking
  // at the screen.
  const awaySinceRef = useRef(null);
  const [awayEpisode, setAwayEpisode] = useState(null);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      // ── THE AWAY EPISODE ──────────────────────────────────────
      // Opened when the merged state begins, closed only when the artist
      // is back AND nothing is still lost. The second half of that
      // condition matters: a lock that turns into a phone call should
      // report one absence when it is genuinely over, not two halves of
      // one as each shape gives way to the next.
      if (next === INTERRUPTION.BACKGROUNDED_DEGRADED) {
        if (!awaySinceRef.current) awaySinceRef.current = now;
      } else if (awaySinceRef.current && !isCapabilityLost(next)) {
        const awayForMs = now - awaySinceRef.current;
        awaySinceRef.current = null;
        setAwayEpisode({ returnedAt: now, awayForMs });
        logHealthEvent('away_episode_ended', { awayForMs });
      }
  ...
  return { state, suspendedAt, suspendedForMs, awayEpisode, lost: isCapabilityLost(state) };
}
