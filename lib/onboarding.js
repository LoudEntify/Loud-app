...
export async function loadOnboarding(userId, role) {
  ...
  try {
    ...
    if (error && !isMissingColumn(error)) {
      // A real failure (network, RLS). Fall through to local so the
      // person still gets a working flow.
...
async function persist(userId, state) {
  ...
  try {
    const { error } = await getSupabase()
      ...
      .eq('id', userId);
    if (error && !isMissingColumn(error)) {
      console.warn('[onboarding] progress not saved to the profile', error.message);
    }
