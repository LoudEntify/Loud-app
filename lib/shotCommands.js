...
export function buildShotCommand({
  showId,
  slot,
  // WHO fired it, as a user id rather than a slot letter.
  //
  // Slot is a position in one show: slot 'b' tonight and slot 'b' next
  // week are different people, so slot alone cannot attribute anything
  // across shows.
  //
  // ── WHAT THIS IS FOR, AND WHAT IT IS NOT FOR ──────────────
  // Provenance and auditing: being able to answer "who fired this cut"
  // about a row that already exists.
  //
  // NOT per-artist director models. What the director learns is camera
  // work — cuts, transitions, technique between feeds — and that is the
  // same skill whoever is performing, so both performers' overrides feed
  // ONE pool. Nothing downstream should start splitting the training
  // signal by this column.
  artistId = null,
  shotKey,
  fromShotKey = null,
  sourceRole,
...
export function createStaccatoSequencer({ room, showId, slot, artistId = null, availableRoles, resolveTargetFor, showPhase }) {
  let timer = null;
  let poolIndex = 0;
...
async function logShotCommand(command) {
  ...
  await supabase.from('shot_commands').insert({
    ...
    slot: command.slot,
    artist_id: command.artistId ?? null,
    shot: command.shot,
