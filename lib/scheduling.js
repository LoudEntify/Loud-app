...
// app/api/performer/join-show).
//
// ⚠️ IMPORTED, then re-exported. NOT `export { … } from './showWindow'`.
//
// That distinction crashed the artist console, so it is worth spelling
// out: `export { x } from './y'` is a pure re-export. It makes `x`
// available to IMPORTERS of this module and creates NO LOCAL BINDING —
// so `windowClosesAt` inside nextUpcomingShow below was an undefined
// identifier, and every caller got
// `ReferenceError: windowClosesAt is not defined` the moment the filter
// ran. It is valid syntax, so nothing failed at build time.
//
// Importing first and re-exporting the local bindings does both jobs:
// the functions are in scope HERE, and still exported for the fifteen
...
// ⚠️ A PURE RE-EXPORT, which the warning at the top of this file says
// crashed the artist console. It is safe HERE and only because of one
// condition: humanCountdown is never CALLED inside this module, so no
// local binding is needed. If anything in this file ever calls it,
// change this to `import { humanCountdown } from './showWindow.js'`
// followed by `export { humanCountdown }` -- or it becomes an undefined
// identifier at runtime with no build-time error, exactly as before.
//
// humanCountdown lives in lib/showWindow.js and is re-exported here so
// every existing caller keeps its import. It moved because this module
// imports the Supabase client, which cannot load in plain Node -- so a
// pure string formatter living here was untestable, and the whole
// window-tests suite crashed on importing it.
export { humanCountdown } from './showWindow.js';
...
export async function sweepClosedShows(shows, now = Date.now()) {
  ...
  try {
    // ITEM 3 -- an unattended end, LABELLED as one.
    //
    // actual_ended_at here is an UPPER BOUND, not the real end: all this
    // knows is that the window has closed, and the show may have stopped
    // an hour earlier. ended_by = 'window_sweep' is what stops that
    // upper bound being read as a fact on the 21st, where it would
    // silently inflate every derived duration.
    //
    // The alternative -- leaving actual_ended_at NULL for an unattended
    // show -- is honest too, but then the show has no end at all and
    // every duration query drops it entirely. A labelled inference is
    // usable; an unlabelled one is indistinguishable from a measurement,
    // and a missing one is indistinguishable from a show that never
    // happened.
    //
    // `.is('actual_ended_at', null)` so a sweep can never overwrite a
    // real end an artist already recorded. The sweep runs over shows
    // that are not yet 'ended', so this should not fire -- but the two
    // writes race in exactly one case worth guarding: the artist presses
    // End Show while a sweep is already in flight.
    const endedAt = new Date(now).toISOString();
    const { error } = await getSupabase()
      .from('shows')
      .update({ state: 'ended', actual_ended_at: endedAt, ended_by: 'window_sweep' })
