  ...
  useEffect(() => {
    ...
    if (!lost) {
      if (announcedRef.current) {
        ...
        publishAwaySignal(room, { away: false, reason: reason || null });
      }
      return undefined;
    }
    ...
    if (!announcedRef.current) {
      ...
      publishAwaySignal(room, { away: true, reason: reason || null });
    }
    // Renewal, so a loss that outlives the TTL is not silently forgiven
    // while it is still happening.
    const timer = setInterval(() => {
      publishAwaySignal(room, { away: true, reason: reason || null });
    }, RENEW_MS);
    return () => clearInterval(timer);
  }, [room, lost, reason, enabled]);
