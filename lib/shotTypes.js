...
export const SHOT_TYPES = {
  ...
  // ───────── B-ROLL CLIP (a file, not a camera) ─────────
  //
  // JUDGMENT CALL, and the reason is concrete rather than aesthetic: this
  // is a NEW shot rather than `bRoll` above being taught to prefer a live
  // clip.
  //
  // `bRoll` is referenced in two places that would have been poisoned by
  // that change:
  //   * staccato's pool (`['wide', 'closeUp', 'bRoll']`) — the sequencer
  //     would hard-cut INTO a playing clip every 500ms;
  //   * NEAREST_SHOT_FOR_ROLE.side — picking the side camera directly
  //     from the feed strip would resolve to the clip instead.
  // Both are silent, both only appear mid-show, and both are the kind of
  // thing that gets diagnosed as "b-roll is broken" rather than as a
  // shot-registry change. Two shots, one family: B-ROLL is still the side
  // camera framed on hands and instrument; B-ROLL CLIP is the file.
  //
  // transform: null — a clip is already framed. A produced 16:9 or 9:16
  // clip cropped to the "hands zone" would be a crop of somebody else's
  // composition.
  //
  // strictSource — see resolveSourceRole. Without it, cueing this shot
  // with no clip playing falls through to `availableRoles[0]`, which is
  // the artist's own camera: tapping B-ROLL CLIP would cut to their face.
  // That is the exact failure this whole round exists to prevent, so the
  // shot refuses to resolve at all rather than resolving to the wrong
  // thing.
  bRollClip: {
    label: 'B-Roll Clip',
    category: 'static',
    ...
    strictSource: true,
    transform: null,
    // Fade is legal here where it is not between two cameras: the
    // ghosting problem that made 'cut' the default everywhere else comes
    // from blending two near-identical framings of the same subject. A
    // clip and a camera are different pictures entirely, so a fade
    // resolves cleanly — and a cut is still available to the rules
    // engine when the outgoing shot only allows one.
    allowedTransitions: ['cut', 'fade'],
    defaultTransition: 'cut',
  },
...
export function resolveSourceRole(shotKey, availableRoles, currentRole = null) {
  ...
  // strictSource: no fallback, ever. A shot that names a source which is
  // not live must resolve to NOTHING rather than to whatever happens to
  // be first in the list.
  //
  // This exists for bRollClip and the failure it prevents is specific:
  // without it, tapping B-ROLL CLIP with no clip playing returns
  // `availableRoles[0]` — the artist's own camera — so the director
  // would cut to the performer's face while believing they had cut to a
  // clip. Every caller already handles a null sourceRole by skipping the
  // cut (staccato's `if (!sourceRole) return`, cueDirector's fallback
  // path), so refusing is a supported answer, not an edge case.
  if (shot.strictSource) return null;
...
export const NEAREST_SHOT_FOR_ROLE = {
  ...
  close: 'closeUp',
  side: 'bRoll',
  // A b-roll track has exactly one shot that can legally frame it, so
  // this mapping is not a guess like the four above it.
  broll: 'bRollClip',
};
