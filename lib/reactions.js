 ...
 */
// ITEM 4 -- viewerId and roomName are the two keys pilot2_04 adds.
//
// viewer_id is what makes a reaction joinable to the person who made it,
// and therefore to their watch time: "did the people who stayed longest
// react more" is not answerable without it. room_name resolves the
// long-standing ambiguity in this table's own show_id, which holds
// EITHER the show uuid or the room name depending on what the client had
// resolved (LiveDemo passes `showId || roomName`) -- carrying the room
// explicitly means the 21st's queries stop having to guess.
//
// Both are optional at this layer. A reaction from a device with no
// storage, or before the show row resolved, is still a real reaction and
// must still be recorded; it simply cannot be attributed.
export function logReaction({ showId, emoji, offsetMs, tokensSpent = 0, viewerId = null, roomName = null }) {
  try {
