...
// ── THE VERSUS VOTE (Sunday 20 September) ─────────────────────
//
// A vote is a `choice` prompt and nothing more. That is the whole
// design: prompt_responses already enforces ONE ROW PER VIEWER PER
// PROMPT with last-one-wins (the unique index fixed on 12 Sept), which
// is exactly the property a vote needs and the property that stops one
// enthusiastic tapper outvoting three people. Reusing it means Sunday's
// vote runs on a path that has been device tested rather than on a new
// one written two days out.
//
// DELIBERATELY NOT IN SHOW_PROMPTS. The operator's questions panel is a
// list of things to ask during the show; the vote is a separate control
// that opens and closes. Keeping it out of that array is what stops it
// being pushed by accident in the run of questions.
//
// ⚠️ THE BODY IS THE DISCRIMINATOR. The viewer's card behaves
// differently for a vote -- it stays on screen so the vote can be
// changed, rather than acknowledging and clearing like a question -- and
// the client has to recognise a vote it received over the data channel
// OR fetched from the catch-up list, which returns only
// id/kind/body/options/pushed_at. Matching the body is the only marker
// that survives both routes without a migration to widen
// show_prompts.source, whose CHECK allows only 'saved' | 'composed'.
export const VERSUS_VOTE = {
  key: 'versus_vote',
  kind: 'choice',
  body: 'Who is winning this round?',
  // The stage names, in slot order: A then B.
  options: ['Artistwon', 'Artisttoo'],
};

/** Is this prompt the Versus vote rather than a question? */
export function isVersusVote(prompt) {
  return !!prompt && typeof prompt.body === 'string' && prompt.body === VERSUS_VOTE.body;
}
